...
export default function HomePage() {
  ...
  const filterProduct = (selectedCategory === "All") ? allProduct : allProduct.filter(product => product.category === selectedCategory)
  useEffect(() => {
    const setCategories = new Set(allProduct.map((p) => p.category));
    setAllCategory([...allCategory, ...setCategories]);
  }, [])

  // const FilterProduct = allProduct.filter((product) => product.name.toLowerCase().includes(serch.toLowerCase()) || product.category.toLowerCase().includes(serch.toLowerCase()));
      ...
      <main className="flex-1 p-6">
        <div className="flex flex-wrap items-center justify-between mb-6">
          <div className="flex flex-wrap gap-2">
            {["All", "electronics", "fashion", "footwear", "beauty"]
              .map((category) => (
                <button
                  key={category}
                  onClick={() => {
          ...
          <div className="flex items-center space-x-4 w-full max-w-md">
            <div className="flex border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-purple-500">
              <input
                type="text"
                // placeholder="Search products..." onChange={(event) => setSerch(event.target.value)}
